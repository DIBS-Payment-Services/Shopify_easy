...
class EasyService implements EasyServiceInterface
{
        ...
        if ($this->customerAddressValidation($checkoutObject)) {
            ...
            if ($checkoutObject->getCompany()) {
                $consumerData['company'] = [
                    'name' => $checkoutObject->getCompany(),
                    'contact' => [
                        'firstName' => $firstName,
                        'lastName' => $lastName
                    ]
        ...
        foreach ($checkoutObject->getLineItems() as $item) {
            ...
            $items[] = array(
                ...
                'name' => strip_tags(str_replace(array(
                    '\'',
                    '&'
                ), '', $this->trimProductName($item['title']))),
        ...
        if (! empty(($checkoutObject->getShippingLines()))) {
            ...
            $shippingLine = [
                ...
                'name' => strip_tags(str_replace(array(
                    '\'',
                    '&'
                ), '', $this->trimProductName($current['title']))),
                'quantity' => 1,
                'unit' => 'pcs',
                'unitPrice' => $unitPrice,
                'taxRate' => $taxRate,
    ...
    protected function customerAddressValidation(\App\CheckoutObject $checkoutObject)
    {
        ...
        if (! empty($checkoutObject->getIso2countryCode()) && ! empty($checkoutObject->getPostalCode()) && ! empty($checkoutObject->getCity()) && (! empty($checkoutObject->getAddressLine1()) || ! empty($checkoutObject->getAddressLine2()))) {
            return true;
        } else {
            return false;
        }
    ...
    {
        return [
            'total_price' => $request->get('x_amount'),
